...
| トラック | いつ | 中身 |
| --- | --- | --- |
| **通常** | ふだん | ジャズ／ファンク。vi-ii-V-I に16分スイングとファンクベース。132 BPM |
| **前兆** | ゲージ62%以上 | メロディを抜き、ライザーとタム回しで不安を積む。150 BPM |
| **ボーナス** | ボーナス中 | **4つ打ち＋ガロップベース＋オフビートのコード刻み**。8小節構成。172〜188 BPM |

ボーナス曲は8小節で「呼びかけ → 応答 → 展開 → 着地 → 1オクターブ上で再提示 → 頂点 → 下降」
という流れを作ってあり、実測 RMS が通常曲の **2.7倍** に跳ね上がります。

作るうえで気をつけたこと（最初の版はここを外していました）:

- **休符を入れる** — 16分を詰め続けると歌にならない。平均 6.6音/小節に落とし、
  同じリズムを8小節中4小節で繰り返してフックにしている
- **声部を近くで動かす** — 和音を転回させ、隣のコードとの移動量を最大11半音に抑えた
  （転回を無視すると21半音跳ねて、何を弾いているのか輪郭が取れなくなる）
- **伸ばす音を作る** — エンベロープに `hold` を足した。減衰するだけだと全部が撥弦音になる
- **アルペジオを拍に乗せる** — 構成音3つを16ステップに敷くと 3対4 で拍とぶつかるので、
  8ステップ1周のパターンにした
- **リードにローパスを通す** — 生のノコギリ波は高音域で耳に刺さる

通常時BGMはさらに作り直しています。最初の版は「コードをそのまま上下するだけ」の
メロディで、裏拍がひとつも無く、コード刻みも無いスカスカな構成でした。

- 進行を **Am-F-C-G**（いわゆる4コード）から **Am7-Dm7-G7-Cmaj7**（vi-ii-V-I）へ
- ルートをベースに任せたルートレスボイシングにして、声部の移動を3半音以内に収めた
- メロディは9th/11th/13thを積極的に使い、非和声音の比率を 14% → 44% に。
  発音も裏拍主体（拍頭5：裏拍11）にして、4小節とも違うリズムにした
- ベースをルート置きから、オクターブ跳躍と下降を含むファンクラインへ
- 裏拍3発のコード刻みを追加（以前は伴奏が一切なかった）
- 16分を 0.14 ぶん後ろに倒す**スイング**を入れて、機械的な等間隔から外した
